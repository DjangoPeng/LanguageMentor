...
def handle_conversation(message, history):
    return conversation_agent.respond(message)
...
with gr.Blocks(title="LanguageMentor 英语私教") as language_mentor_app:
    with gr.Tab("对话练习"):
        ...
        conversation_chatbot = gr.Chatbot(
            placeholder="<strong>你的英语私教 DjangoPeng</strong><br>想和我聊什么话题都可以，记得用英语哦！",
            height=800,
        )
    ...
    with gr.Tab("场景训练"):
        ...
        # scenario_output = gr.Markdown()  # 显示选择的场景
        scenario_chatbot = gr.Chatbot(
            placeholder="<strong>你的英语私教 DjangoPeng</strong><br>选择场景后开始对话吧！",
            height=800,
        )
